pipeline {
    ...
    stages {
        ...
        stage("running the docker images"){
            steps{
               sh 'docker run -dit --name springpet_$IMAGE_TAG -p 8081:8080 ullagallu/spring_petclinic:$IMAGE_TAG'
               sh 'docker run -dit --name springpet_latest -p 8082:8080 ullagallu/spring_petclinic:latest'
            }
        }
    }
}
